...
<div class="content-wrapper">
          <div class="row">
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div id="parent_{{ $checkIfTurnedOn->id }}" data-itemid="{{ $checkIfTurnedOn->id }}" data-db-table="spotify_cron_setter" data-db-row="state" class="d-inline-block" data-toggle="tooltip" data-placement="auto" title="" data-original-title="Turn ON/OFF the processing of Artist.">
            ...
            <h2 class="text-center mb-3"><u>Claimed Artists In Progress</u></h2>
            <p class="text-muted text-center">Below you can see the currently processing/processed claim and social link check for artists</p>
                               ...
                               <div class="table-responsive">
                                ...
                                <p class="text-muted mt-2">{!! $paginationdisplay !!}
                                 ...
                                 <table class="table table-hover resulttable myeditabletable">
                                   <thead>
                                     <tr>
                                       ...
                                       <th class="text-center width120">Items left</th>
                                 ...
                                 @foreach ($processedresults as $processedresult)
                                     <tr id="parent_{{ $processedresult->id }}" data-itemid="{{ $processedresult->id }}" data-db-table="spotify_artists_claim_queue" {!! $processedresult->inprogress=='0' ? 'class="myeditablerow successful-bg"' : ($processedresult->inprogress==1 ? 'class="myeditablerow waiting-bg"' : ($processedresult->inprogress==2 ? 'class="myeditablerow processing-bg"' : 'class="myeditablerow problem-bg"'))!!}>
                                                           
                                       <td class="text-center">
                                        <a href="{{ config('myconfig.config.server_url')  }}admin/localdatabase{{ $processedresult->url }}" target="_blank">
                                            {{ $processedresult->id }}
                                        </a>
                                       </td>
                                       ...
                                       <td class="text-center">{{ $processedresult->item_count>0 ? number_format($processedresult->item_count) : 'Uknown yet' }}</td>
                                       
                                       <td class="text-center itemsleftfield">{!! ($processedresult->item_count>0 && ($processedresult->inprogress=='0' || $processedresult->inprogress=='2'))  ? number_format($processedresult->item_left).'<br>('.(100-(floor(($processedresult->item_left/$processedresult->item_count)*100))).'% done)' : 'Uknown yet' !!}</td>
                                 ...
                                 <div class="text-center mt-3">
                                   <a href="{{ config('myconfig.config.server_url')  }}admin/claimedinprogress">SHOW ALL CLAIM CHECKS</a>
                                </div>
                                 @endif
